 ...
 */
public class Generat extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		JobseakerDaoImpl jd=new JobseakerDaoImpl();
		Jobseaker jobseaker=jd.UserCV(emailId);
		 Document resume = new Document(PageSize.A4);
         PdfWriter.getInstance(resume, new FileOutputStream(filename));
         resume.open(); 
         resume.addCreator("Binod by Demo.java");
         resume.addAuthor("Binod Suman");
         resume.addTitle("First PDF By Binod");
         resume.addTitle("Curriculum Vitae ");
		   
		   Image image = Image.getInstance("d:/imgJobseaker/"+jobseaker.getId_details()+".jpg");
		   ...
		   resume.add(image);
		      LineSeparator line;
	            ...
	            Paragraph name = new Paragraph("Name : " +(String) jobseaker.getName());
	            name.setAlignment(Paragraph.ALIGN_LEFT);
	            resume.add( name );
	            Paragraph email = new Paragraph("Email :"+(String) jobseaker.getEmail());
...
//	            resume.add(name9);
		...
		response.setContentType("APPLICATION/OCTET-STREAM"); //for binary data download
